...
export default class MetadataPanel {
  ...
  setMetadata(metadata = {}) {
    ...
    const slugPreview = source.slug
      ? this.#slugify(source.slug)
      : this.#slugify(this.fields.title?.value || '');
      ...
      this.tagSuggestionsEl.addEventListener('click', (event) => {
        ...
        if (added) {
          this.tagSuggestions = this.tagSuggestions.filter(
            (tag) => tag !== btn.dataset.tagSuggestion
          );
          this.#renderTagSuggestions();
          this.#notify();
        }
  ...
  #normaliseTags(value) {
    if (Array.isArray(value)) {
      return value
        ...
        .filter(
          (item, index, array) =>
            index === array.findIndex((other) => other.toLowerCase() === item.toLowerCase())
        );
    }
