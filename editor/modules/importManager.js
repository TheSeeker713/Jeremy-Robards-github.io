...
const JSON_FIELDS = [
  ...
  { key: 'tags', label: 'Tags', aliases: ['tags', 'keywords'] },
  {
    ...
    aliases: ['published_at', 'publishdate', 'date'],
  },
...
export default class ImportManager {
  ...
  async #processPdf(file, buffer) {
    ...
    const headingBlock = blocks.find(
      (block) => block.type === 'text' && block.variant?.startsWith('heading')
    );
    if (headingBlock) {
  ...
  #normaliseMarkdownMetadata(data = {}, content = '', fileName = '') {
    const metadata = {
      ...
      published_at: this.#coerceDate(
        data.published_at || data.publishDate || data.publishedAt || data.date
      ),
      excerpt: '',
      ...
      case 'header':
        return [
          this.#createTextBlock(
            ...
            `heading-${Math.min(section.level || 2, 4)}`
          ),
        ];
      case 'quote':
  ...
  #createDraft({
    ...
    const baseMetadata = {
      ...
      published_at: this.#coerceDate(
        metadata.published_at || metadata.publishDate || metadata.date
      ),
  ...
  async #promptJsonMapping(keys, initialMapping = {}) {
    if (!this.modalRoot) {
      const titleKey = window.prompt(
        'JSON mapping: enter the key for title',
        initialMapping.title || ''
      );
      const bodyKey = window.prompt(
        'JSON mapping: enter the key for body',
        initialMapping.body || ''
      );
  ...
  #decodeSnippet(buffer, length = 512) {
    const view =
      buffer instanceof ArrayBuffer
        ? new Uint8Array(buffer, 0, Math.min(length, buffer.byteLength))
        : new Uint8Array(buffer.buffer, buffer.byteOffset, Math.min(length, buffer.byteLength));
  ...
  #readSignature(buffer, length = 5) {
    const bytes = new Uint8Array(
      buffer instanceof ArrayBuffer
        ...
        : buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + length)
    );
