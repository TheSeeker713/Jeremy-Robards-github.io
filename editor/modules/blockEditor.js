...
export default class BlockEditor {
  ...
  duplicateBlock(id) {
    ...
    const clone = structuredClone
      ? structuredClone(this.blocks[index])
      : JSON.parse(JSON.stringify(this.blocks[index]));
    clone.id = this.#id();
  ...
  #renderImageBlockControls(block) {
    ...
    buttonRow.append(
      this.#controlButton(
        block.src ? 'Replace image' : 'Select image',
        'pick-image',
        'button--primary'
      ),
  ...
  async #replaceImageFromPicker(id) {
    ...
    try {
      ...
      if (picked.reduction > 0) {
        this.toast?.show(
          `✅ Image optimized (${picked.reduction}% smaller, ${picked.processedDimensions.width}×${picked.processedDimensions.height})`
        );
      } else {
        this.toast?.show(
          `✅ Image loaded (${picked.processedDimensions.width}×${picked.processedDimensions.height})`
        );
      }
    } catch (error) {
      this.toast?.show(error.message || 'Image selection cancelled.');
